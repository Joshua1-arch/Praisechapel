...
const Sermons = () => {
  ...
  return (
    <div className="flex flex-col">
      {/* Hero Section */}
      <section className="relative py-24 md:py-32 bg-[var(--bg-body)] overflow-hidden border-b border-[var(--border-color)]">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 text-center space-y-6">
          ...
          <div className="max-w-2xl mx-auto space-y-4 mt-12">
            <div className="flex items-center bg-[var(--bg-input)] border border-[var(--border-color)] rounded-2xl p-2 shadow-2xl focus-within:border-brand-red transition-all">
              <Search className="text-[var(--text-muted)] ml-4" size={24} />
              <input
                type="text"
                placeholder="Search by topic, scripture, or speaker..."
                className="bg-transparent border-none focus:ring-0 text-[var(--text-main)] w-full px-4 py-3 text-lg outline-none placeholder:text-[var(--text-muted)]"
                value={searchTerm}
            ...
            <div className="flex flex-wrap justify-center gap-2">
              {speakers.map(speaker => (
                <button
                  ...
                  className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border ${selectedSpeaker === speaker
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex justify-between items-end mb-12">
            <div>
              ...
              <h3 className="text-4xl font-black text-[var(--text-main)] uppercase tracking-tight">The Life of Prayer</h3>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="group relative aspect-[3/4] rounded-2xl overflow-hidden shadow-2xl cursor-pointer border border-[var(--border-color)]">
                <img
      ...
      <section className="py-24 bg-[var(--bg-body)] border-t border-[var(--border-color)]">
        <div className="max-w-7xl mx-auto px-6">
          <h2 className="text-4xl font-black text-[var(--text-main)] mb-12 uppercase tracking-tight">Recent <span className="text-brand-green">Word</span></h2>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredSermons.map((msg, i) => (
              <div key={i} className="group bg-[var(--bg-card)] rounded-3xl overflow-hidden border border-[var(--border-color)] hover:border-brand-red/50 transition-all shadow-xl">
                ...
                <div className="p-8 space-y-4">
                  ...
                  <h3 className="text-xl font-black text-[var(--text-main)] group-hover:text-brand-red transition-colors uppercase tracking-tight leading-none">{msg.title}</h3>
                  <p className="text-[var(--text-muted)] font-bold text-sm">with {msg.speaker}</p>
                  ...
                  <div className="flex gap-2 pt-4">
                    ...
                    <button className="p-3 rounded-xl bg-[var(--bg-input)] text-[var(--text-muted)] hover:text-white hover:bg-brand-red transition-all">
                      <Music size={18} />
                    </button>
                    <button className="p-3 rounded-xl bg-[var(--bg-input)] text-[var(--text-muted)] hover:text-white hover:bg-brand-red transition-all">
                      <FileText size={18} />
