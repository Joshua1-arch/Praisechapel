  ...
  return (
    <div className="flex flex-col bg-[var(--bg-body)] min-h-screen">
      <section className="relative py-20 bg-[var(--bg-card)] border-b border-[var(--border-color)] overflow-hidden text-center">
        <h1 className="text-5xl md:text-6xl font-black text-[var(--text-main)] uppercase tracking-tight">Prophetic <span className="text-brand-red">Blog</span></h1>
        <p className="text-[var(--text-muted)] mt-4 max-w-xl mx-auto font-medium">Wisdom and insights for your spiritual journey.</p>
      </section>

      <section className="py-20 px-6 max-w-7xl mx-auto w-full">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {posts.map((post, i) => (
            <div key={i} className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-[2rem] overflow-hidden hover:border-brand-red/50 transition-all group">
              <div className="h-48 bg-[var(--bg-input)] relative">
              ...
              <div className="p-8 space-y-4">
                ...
                <h3 className="text-xl font-black text-[var(--text-main)] uppercase tracking-tight group-hover:text-brand-green transition-colors">{post.title}</h3>
                <p className="text-[var(--text-muted)] text-sm font-medium leading-relaxed">Discover deep spiritual truths and revelations in this latest exaltation from our leadership.</p>
