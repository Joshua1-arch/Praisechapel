    ...
    <div className="flex flex-col">
      ...
      <section className="py-24 bg-[var(--bg-body)]">
        <div className="max-w-7xl mx-auto px-6">
          <div className="relative rounded-[2rem] overflow-hidden bg-[var(--bg-card)] border border-[var(--border-color)] shadow-2xl group">
            <div className="grid grid-cols-1 lg:grid-cols-2">
              <div className="aspect-video lg:aspect-auto overflow-hidden border-r border-[var(--border-color)]">
                <img
              ...
              <div className="p-8 md:p-16 flex flex-col justify-center space-y-8">
                ...
                <h2 className="text-4xl font-black text-[var(--text-main)] uppercase tracking-tight leading-none">Prophetic Prayer Meeting</h2>
                <div className="space-y-4">
                  <div className="flex items-center gap-3 text-[var(--text-muted)] font-bold uppercase tracking-widest text-xs">
                ...
                <div className="flex gap-4 pt-4">
                  ...
                  <button
                    ...
                    className="p-4 rounded-xl bg-[var(--bg-input)] text-[var(--text-muted)] hover:text-white hover:bg-brand-red transition-all flex items-center gap-2"
                    title="Add to Calendar"
                  >
                    <CalendarIcon size={20} />
                    <span className="text-[10px] font-black uppercase tracking-widest hidden md:block">Add to Calendar</span>
                  </button>
                  <button className="p-4 rounded-xl bg-[var(--bg-input)] text-[var(--text-muted)] hover:text-white hover:bg-brand-red transition-all">
                    <Share2 size={24} />
                  </button>
                </div>
              </div>
      ...
      <section className="py-24 bg-[var(--bg-body)] border-t border-[var(--border-color)]">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col md:flex-row justify-between items-center mb-16 gap-8">
            <h3 className="text-4xl font-black text-[var(--text-main)] uppercase tracking-tight">Gathering <span className="text-brand-green">Calendar</span></h3>
            <div className="flex gap-2 p-1 bg-[var(--bg-input)] rounded-full border border-[var(--border-color)]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {upcomingEvents.map((event, i) => (
              <div key={i} className="group bg-[var(--bg-card)] p-8 rounded-[2.5rem] border border-[var(--border-color)] hover:border-brand-green/50 transition-all flex flex-col justify-between space-y-8">
                <div className="space-y-6">
                  <div className="flex justify-between items-start">
                    <div className="w-16 h-16 bg-[var(--bg-body)] rounded-2xl flex flex-col items-center justify-center border border-[var(--border-color)] group-hover:bg-brand-red group-hover:border-brand-red transition-all shadow-xl">
                      <span className="text-[var(--text-muted)] text-[10px] font-black uppercase group-hover:text-white transition-colors">{event.date.split(' ')[0].substring(0, 3)}</span>
                      <span className="text-white text-2xl font-black">{event.date.split(' ')[1]}</span>
                    </div>
                    <div className="flex flex-col items-end gap-2">
                      <span className="px-3 py-1 rounded-full bg-[var(--bg-body)] text-brand-green text-[10px] font-black uppercase tracking-widest border border-[var(--border-color)]">{event.cat}</span>
                      <button
                        onClick={() => addToCalendar(event)}
                        className="p-2 rounded-lg bg-[var(--bg-body)] border border-[var(--border-color)] text-[var(--text-muted)] hover:text-brand-red transition-all"
                        title="Add to Calendar"
                      >
                  ...
                  <div>
                    <h4 className="text-xl font-black text-[var(--text-main)] group-hover:text-brand-green transition-colors uppercase tracking-tight leading-none">{event.title}</h4>
                ...
                <button className="flex items-center justify-between w-full p-4 rounded-xl bg-[var(--bg-body)] hover:bg-[var(--bg-input)] text-[var(--text-muted)] hover:text-[var(--text-main)] text-xs font-black uppercase tracking-widest transition-all border border-[var(--border-color)]">
                  Register
